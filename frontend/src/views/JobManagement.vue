...
import { MapPin, Banknote, BriefcaseBusiness, Users, Clock, CheckCircle, XCircle } from 'lucide-vue-next'
...
<template>
  <div class="min-h-screen bg-white">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
          <div @click="setStatusFilter('all')" class="cursor-pointer">
            <DashboardStatCard
              ...
              :icon="Users"
              :cardClass="`h-[100px] md:h-[140px] bg-blue-50 text-blue-600 text-sm border-0 shadow-md transition-transform hover:scale-105 ${statusFilter === 'all' ? 'ring-2 ring-blue-500' : ''}`"
          ...
          <div @click="setStatusFilter('pending')" class="cursor-pointer">
            <DashboardStatCard
              ...
              description="Applicants"
              :icon="Clock"
              :cardClass="`h-[100px] md:h-[140px] bg-yellow-50 text-yellow-600 text-sm border-0 shadow-md transition-transform hover:scale-105 ${statusFilter === 'pending' ? 'ring-2 ring-yellow-500' : ''}`"
          ...
          <div @click="setStatusFilter('approved')" class="cursor-pointer">
            <DashboardStatCard
              ...
              description="Applicants"
              :icon="CheckCircle"
              :cardClass="`h-[100px] md:h-[140px] bg-green-50 text-green-600 text-sm border-0 shadow-md transition-transform hover:scale-105 ${statusFilter === 'approved' ? 'ring-2 ring-green-500' : ''}`"
          ...
          <div @click="setStatusFilter('rejected')" class="cursor-pointer">
            <DashboardStatCard
              ...
              description="Applicants"
              :icon="XCircle"
              :cardClass="`h-[100px] md:h-[140px] bg-red-50 text-red-600 text-sm border-0 shadow-md transition-transform hover:scale-105 ${statusFilter === 'rejected' ? 'ring-2 ring-red-500' : ''}`"
    ...
    <section class="mx-auto px-[8vw] md:px-[12vw] py-8">
      <div class="mb-6 flex flex-col gap-y-4 md:flex-row items-start md:items-center justify-between text-sm md:text-base">
        <div>
          <h2 class="text-2xl font-bold text-gray-900">
            {{ statusFilter === 'all' ? 'All Applicants' : `${statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)} Applicants` }}
      ...
      <div v-if="filteredApplicants.length === 0" class="text-center py-12 bg-white rounded-lg shadow-sm">
