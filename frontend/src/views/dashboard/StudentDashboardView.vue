...
// import { getAppliedJobs, getBookmarkedJobs, getRecentlyViewedJobs } from '@/api/jobs'
...
async function fetchJobs() {
  try {
    ...
    // appliedJobs.value = await getAppliedJobs()
    ...
    import('@/data/mockJobs').then(({ mockJobs }) => {
      ...
      bookmarkedJobs.value = mockJobs.slice(5, 7)
        ...
        <DashboardStatCard
          ...
          :cardClass="
            openSection === 'recentlyViewed'
              ? 'bg-yellow-600 rounded-md text-white shadow-md overflow-hidden'
              : 'bg-yellow-400 rounded-md text-white shadow-md overflow-hidden'
          "
        />
      </div>
    </div>
    ...
    >
      ...
      <div class="max-h-[600px] overflow-y-auto pr-2">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
          <JobCard v-for="job in appliedJobs" :key="job.jobId" :job="job" @select="handleSelect" />
        </div>
      </div>
    </section>
