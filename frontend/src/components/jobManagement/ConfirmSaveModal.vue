...
<template>
  <div class="fixed inset-0 flex items-center justify-center z-50">
    ...
    <div class="relative bg-white rounded-lg overflow-y-auto shadow-lg z-50 max-w-2xl w-full mx-4">
      ...
      <X class="absolute text-gray-500 top-4 right-4 cursor-pointer hover:text-gray-600" @click="$emit('handleModalClick', 'cancel')" />
      ...
      <div class="p-4 max-h-96 overflow-y-auto max-h-[30vh]">
        ...
        <div v-if="groupedChanges.approved.length > 0" class="mb-4">
          ...
          <ul class="space-y-1">
            <li v-for="applicant in groupedChanges.approved" :key="applicant.id" class="text-sm border-2 border-green-500 rounded-md px-3 py-2">
            {{ applicant.name }}
            </li>
          </ul>
        </div>
        ...
        <div v-if="groupedChanges.rejected.length > 0" class="mb-4">
          <h4 class="font-medium mb-2 text-red-700">Rejected ({{ groupedChanges.rejected.length }})</h4>
          <ul class="space-y-1">
            <li v-for="applicant in groupedChanges.rejected" :key="applicant.id" class="text-sm border-2 border-red-300 rounded-md px-3 py-2">
            {{ applicant.name }}
            </li>
          </ul>
        </div>
      ...
      <div class="p-4 border-t flex justify-center gap-2">
        <button class="bg-gray-400 rounded-lg px-4 py-2 text-white hover:cursor-pointer hover:bg-gray-500" @click="$emit('handleModalClick', 'cancel')">Cancel</button>
        <button class="bg-blue-500 rounded-lg px-4 py-2 text-white hover:cursor-pointer hover:bg-blue-600" @click="$emit('handleModalClick', 'save')">Confirm</button>
      </div>
    </div>
  </div>
</template>