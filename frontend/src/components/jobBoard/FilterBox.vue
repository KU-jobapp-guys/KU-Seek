...
const filters = reactive<Partial<Filters>>({})
...
const initializeFilters = () => {
  allFields.forEach(field => {
    if (!(field.key in filters)) {
      filters[field.key as FilterKeys] = ''
    }
  })
}
...
const clearFilters = () => {
  Object.keys(filters).forEach(k => (filters[k as FilterKeys] = ''))
  emit('filtersChanged', filters)
}
</script>
