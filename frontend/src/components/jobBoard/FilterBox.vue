...
onMounted(() => {
  initializeFilters()
  // removed: emit('applyFilter', filters)
})
  ...
  >
    ...
    <div class="p-8 flex flex-col gap-y-8 text-base">
        ...
        <div v-for="dropdown in dropdownConfig" :key="dropdown.key" class="group">
          ...
          <div class="relative">
            <select
              ...
              class="w-full px-4 py-2 bg-white/90 border-2 border-gray-200 rounded-2xl shadow-sm hover:shadow-lg focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 appearance-none cursor-pointer text-gray-700"
            >
      ...
      <div class="flex flex-col sm:flex-row gap-4">
        <button
          @click="applyFilters"
          class="flex-1 bg-gradient-to-r from-blue-600 via-purple-600 to-blue-800 text-white font-bold py-2 px-8 rounded-2xl shadow-lg hover:shadow-xl"
        >
          Apply Filters
        </button>
        ...
        <button
          @click="clearFilters"
          class="sm:flex-none bg-white hover:bg-gray-50 text-gray-700 font-semibold py-2 px-6 rounded-2xl border-2 border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-md"
        >
          Reset
        </button>
      </div>
